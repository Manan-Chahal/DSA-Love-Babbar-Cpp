...
public:
    ...
    int solveMem(vector<vector<int>>& cuboids, int curr, int prev,vector<vector<int> >&dp){
        ...
        // we did prev+1 here becase if prev=-1 then it wille be something
        //dp[2][-1] which is invalid index so we added +1 to make it dp[2][0]
        //which will be a valid index
        // inc / exc pattern
        int incl = 0;
        if(prev == -1 || isSafeToPlace(cuboids[prev],cuboids[curr])){
    ...
    int solveTab(vector<vector<int>>&cuboids){
        ...
        for(int curr = n-1; curr>=0; curr--){
            for(int prev = curr-1; prev>=-1;prev--){
                //we took for(int prev = curr-1; prev>=-1;prev--)
                //because prev is always behind curr, so prev can never be equal to curr
                //and prev can never be greater than curr
                // inc / exc pattern
                int incl = 0;
                if(prev == -1 || isSafeToPlace(cuboids[prev],cuboids[curr])){
                    incl = cuboids[curr][2] + dp[curr+1][curr+1];
                    //it was dp[curr+1][curr] we added one beacuse same problem 
                    //invalid index can come here also; 
                }
                int excl = 0 + dp[curr+1][prev+1];
                //dp[curr+1][prev]; here also we added +1 to prev
                dp[curr][prev+1] = max(incl,excl);
            }
        }
        return dp[0][-1+1];
        //we did +1 to make valid index here
    }
