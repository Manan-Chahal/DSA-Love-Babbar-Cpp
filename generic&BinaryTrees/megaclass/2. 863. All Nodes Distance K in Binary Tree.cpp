...
public:
    ...
    vector<int> distanceK(TreeNode *root, TreeNode *target, int k)
    {
        ...
        while (!q.empty())
        {
            ...
            if (currentLevel == k)
                break; // now my answer nodes should be present in queue itself
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                {
                    // When a search function like .find() fails to locate an element,
                    // it returns the .end() iterator. This allows you to check if a key 
                    //exists in a map by comparing the returned iterator with .end().
